    ...
    if (idStr != null && !idStr.trim().isEmpty()) {
        try {
            ...
            PreparedStatement ps = conexion.prepareStatement(
                "SELECT titulo, descripcion, nivel, contenido, video_url FROM tutoriales WHERE id_tutorial = ? AND id_instructor = ?"
            );
...
<html>
<head>
    <title>Editar Tutorial - Plataforma IA</title>
    <meta charset="UTF-8">
...
<body>
    ...
    <div class="container dashboard-container">
        <div class="row">
            ...
            <div class="col-md-9">
                <h2 class="title">Editar Tutorial</h2>
                <p class="lead mb-4">Modifica tu tutorial y actualízalo para la comunidad</p>

                <div class="form-container">
                    <form action="actualizarTutorial.jsp" method="post">
                        <input type="hidden" name="id_tutorial" value="<%= idTutorial %>">
                        ...
                        <div class="form-group mt-4">
                            ...
                            <textarea name="descripcion" id="descripcion" class="form-control input" rows="3" required><%= descripcion %></textarea>
                        </div>

                        <div class="form-group mt-4">
                            <label for="nivel" class="form-label">Nivel</label>
                            <select name="nivel" id="nivel" class="form-control input" required>
                                <option value="Principiante" <%= "Principiante".equals(nivel) ? "selected" : "" %>>Principiante</option>
                                <option value="Intermedio" <%= "Intermedio".equals(nivel) ? "selected" : "" %>>Intermedio</option>
                                <option value="Avanzado" <%= "Avanzado".equals(nivel) ? "selected" : "" %>>Avanzado</option>
                            </select>
                        </div>

                        <div class="form-group mt-4">
                            <label for="contenido" class="form-label">Contenido del Tutorial (texto)</label>
                            <textarea name="contenido" id="contenido" class="form-control input" rows="10" required><%= contenido %></textarea>
                        </div>
                        ...
                        <div class="form-group mt-4">
                            ...
                            <input type="url" name="video_url" id="video_url" class="form-control input"
                                   value="<%= video_url %>" placeholder="Ej: https://www.youtube.com/embed/...">
